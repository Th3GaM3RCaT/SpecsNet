...
class MonitorTendencias:
    ...
    def verificar_tendencia(self, serial: str, tipo: str) -> Optional[Dict]:
        ...
        if todas_superan and not ya_alertado:
            ...
            self.db_cursor.execute(
                "SELECT Name FROM Dispositivos WHERE serial = ?",
                (serial,)
            )
    ...
    def procesar_actualizacion_dispositivo(self, serial: str, datos: Dict) -> List[Dict]:
        """
...
def verificar_recursos_dispositivo(serial: str, datos: Dict, db_path: str = "specs.db") -> List[Dict]:
    """
